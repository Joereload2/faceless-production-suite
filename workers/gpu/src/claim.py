...
def complete_job(conn, contract, job_id, project_id, owner, output_json, bytes_out, now_iso) -> None:
    conn.execute("BEGIN IMMEDIATE")
    rows = conn.execute(
...
def fail_job(conn, contract, job_id, owner, error_code, error, now_iso) -> None:
    conn.execute(
        contract["sql"]["fail"],
        {
